...
public class BombController {
    ...
    public boolean update(Group group, BulletController bulletController, ParticleController particleController){
        ...
        if(spawn_timer >= SPAWN_RATE){
            ...
            bombs.add(new TargetSprite(bombImage, PiGame.SCR_WIDTH/2, PiGame.SCR_HEIGHT/2, 16, 16, 360*rng.nextFloat(), 2));
            spawn_timer = 0;
        }
            ...
            if(bomb.isAlive()){
                ...
                for (BulletSprite bullet : bulletController.getBulletList()) {
                    if (bomb.intersects(bullet)) {
                        ...
                        particleController.addParticleSystem(new ParticleSystem(20, bomb.getX(), bomb.getY(), 15, 14, 24, ParticleColors.EXPLOSION, new ParticleSystemProperties(5, 4, 10)));
                        return false;
